...
/* Changed the code to get multiple staff ids from the Database */

$staff_ids = false;
if ($this->ci->input->post('staff_id')) {
    $staff_ids = $this->ci->input->post('staff_id');
} elseif ($view_all == false) {
    $staff_ids = array(get_staff_user_id());
}
...
if (is_array($staff_ids)) {
    ...
    if (count($staff_ids) > 0) {
        $staff_ids = implode(',', array_map(function ($staff_id) {
            return get_instance()->db->escape_str($staff_id);
        }, $staff_ids));;
        array_push($where, 'AND staff_id IN(' . $staff_ids . ')');
    }
}
...
if ($filter != 'period') {
    ...
    /* Modifying the code for fetching data with date and time  */
    $start_date = to_sql_date($this->ci->input->post('period-from'), true);
    $end_date   = to_sql_date($this->ci->input->post('period-to'), true);
